...
void Vtb_soc::eval_end_step() {
    VL_DEBUG_IF(VL_DBG_MSGF("+eval_end_step Vtb_soc::eval_end_step\n"); );
#ifdef VM_TRACE
    // Tracing
...
void Vtb_soc___024root__trace_init_top(Vtb_soc___024root* vlSelf, VerilatedVcd* tracep);
...
VL_ATTR_COLD void Vtb_soc::trace(VerilatedVcdC* tfp, int levels, int options) {
    if (tfp->isOpen()) {
        vl_fatal(__FILE__, __LINE__, __FILE__,"'Vtb_soc::trace()' shall not be called after 'VerilatedVcdC::open()'.");
    }
    if (false && levels && options) {}  // Prevent unused
    tfp->spTrace()->addModel(this);
    tfp->spTrace()->addInitCb(&trace_init, &(vlSymsp->TOP));
    Vtb_soc___024root__trace_register(&(vlSymsp->TOP), tfp->spTrace());
}
